...
function isOperationDisabled(op, display) {
  ...
  if (
    digits === "" &&
    !((op >= "0" && op <= "9") || op === "π" || op === "e" || op === "AC")
  )
    return true;
  if (
    op === "bs" &&
    (display.isOperandEmpty() ||
      !(
        (lastOp >= "0" && lastOp <= "9") ||
        lastOp === "π" ||
        lastOp === "e" ||
  ...
  if (op === "." && digits.search(/\./) != -1) return true;
  if (op === "√" && digits.search(/-/) != -1) return true;
  if (op === "AC" && display.isDisplayEmpty()) return true;

  return false;
}

function digitPressed(op, display) {
  if (isOperationDisabled(op, display)) return;
  if (lastOp === "π" || lastOp === "e") return;
  ...
  if (
    ...
    if (digits.length >= display.maxDigits) return;
