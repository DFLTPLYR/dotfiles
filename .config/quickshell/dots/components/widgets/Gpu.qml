    ...
    GridLayout {
        ...
        FontIcon {
            ...
            font.pixelSize: Navbar.config.side
                ? Math.min(gpu.contentWidth, gpu.contentHeight / 2) * 0.6
                : Math.min(gpu.contentWidth / 2, gpu.contentHeight) * 0.6
        }
        ...
        Text {
            ...
            color: Colors.color.primary
            font.pixelSize: Navbar.config.side
                ? Math.min(gpu.contentWidth, gpu.contentHeight / 2) * 0.4
                : Math.min(gpu.contentWidth / 2, gpu.contentHeight) * 0.4
        }
    }
}
